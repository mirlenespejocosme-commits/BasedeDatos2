...
function renderAdmin() {
    ...
    const fotoEl = document.getElementById('presentacion-foto');
    if (portfolioData.foto_perfil && portfolioData.foto_perfil !== "") {
        fotoEl.src = portfolioData.foto_perfil;
...
async function subirFotoPerfil(input) {
    if (input.files && input.files[0]) {
        ...
        const fileName = "perfil_" + file.name.replace(/\s+/g, '_');
        ...
        try {
            ...
            portfolioData.foto_perfil = fileName;
            await saveToGithub();
